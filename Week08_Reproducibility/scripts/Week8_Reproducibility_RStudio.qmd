...
If we have data that is in our working directory and not inside of any other folders, we can use the file name alone to read in the data.
...
For reasons I haven't quite been able to nail down, file paths work a little bit different in R scripts (.R file that only understand the R language) and Quarto files (.qmd files, like we use, and also .Rmd files) in RProjects.

-   For scripts, the R Project folder is the working directory, which is standard.

-   For Quarto (and RMarkdown) files, the folder in which the file is located in the working directory..

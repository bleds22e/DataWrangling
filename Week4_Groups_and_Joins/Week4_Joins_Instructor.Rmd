...
In today's lesson, we will be talking about how to bring multiple
dataframes together into one data frame.
...
the `portalr` package, a package we made to make working with the
(actual) Portal data a bit easier. Run the following code chunk.
...
We want to bind these two dataframes together so that we have *all* of
the rodent data in one dataframe. To do so, we would use `bind_rows()`.
...
the dataframes you want to bind together.
...
combine data, but they are, therefore, the more flexible and useful of the two.

The magic comes because they *match up columns of data based on
unique identifiers in each row of data*.
...
table, and full joins, which keep all rows from both tables, even there isn't a matching row. 

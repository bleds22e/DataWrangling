...
Different programming languages have different syntax (language structure). The `tidyverse` is a package (more accurately, a set of packages) offered in R that all have similar goals and a unified syntax designed to work particularly well with 2-dimensional data (data with rows and column). We call these dataframes in R.
...
Get started with your Assignment. After the set-up, work on Shrub Volume Data Basics a-b.
...
Using “and” means that all of the conditions must be true. Do this in `dplyr`, we can add arguments separated by commas or use the `&`
...
Do Shrub Volume Data Basics e-g.
...
Work on the last of Exercise 1, Shrub Volume.

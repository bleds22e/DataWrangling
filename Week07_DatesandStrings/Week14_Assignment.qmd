---
title: "Assignment 14: Dates and Strings"
...
[View Answer Key](Week7_Assignment_AnswerKey.html){.btn .btn-primary role="button"}
:::
...
a.  Using the `make_date` function and the `mutate()` function, create a new column called "collection_date" that has the year, month, and day that I collected the voucher specimen for each plant. Save this new column in the `vouchers` data frame.
...
c.  Use code to find the last date that I collected a voucher specimen.
d.  Find the span of time (duration) that I was collecting voucher specimens.
e.  Create a column for the day of year (DOY) that each specimen was collected.

### 4. NDVI from the Santa Rita Experimental Range (20 pts)

There is a large [network of phenocams](https://phenocam.nau.edu/webcam/) (cameras set up to take daily images of a landscape to monitor plant phenology) across the US. The Santa Rita Experimental Range (SRER) has one such camera.
...
```{r}
phenocam <- read_csv("data_raw/neon_srer_ndvi_phencom.csv", skip = 17, col_names = TRUE,
                    col_types = "ccncc") %>%
  select(date, local_std_time, contains("mean"), NDVI_c)
```

a.  Create a new column in `phenocam` using the `unite()` function to join the date and time columns together into one column. Separate the date and time with a space.
b.  Convert the datetime column you created in (a) to a POSIXct format. Save this new column to `phenocam`. You can use whichever `lubridate` function you would like.
c.  Calculate the duration of the phenocam dataset provided here.
d.  Create new columns in `phenocam` for the year, the month, and the day of year using `lubridate` functions.
...
Let's ease into our practice working with strings with some lyrics from the first Black woman to ever hit the top of the Country music charts (you can tell when I first wrote this question...).
...
```{r}
dugout <- read_csv("data_raw/elevation_2017.csv") %>%

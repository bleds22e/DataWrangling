...
```{r}
download.file(url = "https://ndownloader.figshare.com/files/5629542",
              destfile = "ACACIA_DREPANOLOBIUM_SURVEY.txt")

download.file(url = "https://ndownloader.figshare.com/files/5629536",
              destfile = "TREE.txt")
```
...
When we open this data, we can see that the data are separated by tabs. We can also see that the dataset is not a `.csv` file but rather a `.txt` (a "text" file).
...
::: callout-tip
## Tip! Inside or Outside the `aes()` Function?

Do you want a feature (e.g., color, fill) to vary by values in a *column in the data frame* (e.g., different groups are plotted in different colors)? Put the argument *inside* of the `aes()` function.\
Example: `aes(color = species)`\

Do you want to set a fixed appearance (e.g., every data point is the same color, regardless of group)? Put the argument *outside* of the `aes()` function.\
...
Other transformations exist to make plots like histograms, bar plots, etc., and can occur as defaults in associated `geoms`. For example, let's say we want to take a look at the number of acacia in each treatment using a bar plot. To do so, we would use [`geom_bar()`](https://ggplot2.tidyverse.org/reference/geom_bar.html).
...
When we want all of the layers to have the same defaults, putting those defaults into the `ggplot` function makes a lot of sense.

However, sometimes we want to plot data from different columns or even data frames on the same graph.

...
Last week, we talked about 1-dimensional data in R: *vectors*!

Vectors are objects in R which contain more than one related data points, which we call elements. They are considered 1D data because we only need to specify one location or position in a given vector to identify an individual data point.

Vectors are the "building blocks" of data in R. We start with vectors because most of the other data objects in R are made out of or operate similarly to vectors.

### Data Frames

The data we will work with most often in this class is 2-dimensional data (data with rows and columns). In R, these 2D data objects are called *data frames*.

In data frames, each column is actually a vector. Combining several vectors together produces a data frame. Let's take the vectors we used in our previous lesson and combine them into a single data object to demonstrate.

First, we will create the vectors.
...
Then, we will combine them into a data frame. Each vector will become a column in the data frame.

```{r}
# create data frame
state_data <- data.frame(states, count, area)
state_data # print data frame

# compare output of structure to environment for data frame and vectors
str(state_data)
```

## The `tidyverse`: What is it?

Different programming languages have different syntax (language structure). The `tidyverse` is a package (more accurately, a set of packages) offered in R that all have similar goals and a unified syntax designed to work particularly well with data frames.

Until now, all of the coding we have done is in the original R language, which is called "base R." The syntax in the `tidyverse` is often different from base R. Both are useful, and many people combine them, which is why we start with base R.

![](images/tidyverse.png){width="40%" fig-align="center"}
...
If you want to learn more about the tidyverse, head over to [www.tidyverse.org](www.tidyverse.org) and browse the site. Below is a brief summary of *some* of the packages I think you might find the most useful. We will be using functions from all of these packages by the end of the course.
...
```{r}
# the arguments in the function are the URL and the file destination (file name)
download.file(url = "https://ndownloader.figshare.com/files/2292172", destfile = "surveys.csv")
download.file(url = "https://ndownloader.figshare.com/files/3299474", destfile = "plots.csv")
download.file(url = "https://ndownloader.figshare.com/files/3299483", destfile = "species.csv")
```
...
![](images/mutate.png){width="50%" fig-align="center"}
...
| The format is `mutate(new_column = content_to_put_in_column)`.
...
![](images/filter.png){width="50%" fig-align="center"}

...
First, run the code chunk below to download the data file.

```{r}
# download "shrub-volume-data.csv" data file
...
Rearrange the following code chunk so that it:
...
```{r}
# rearrange the following lines of code so they run successfully
...
Develop a data processing pipeline for the Portal surveys table that produces a table of data for only the three *Dipodomys* species (DM, DO, DS).

...
```{r}
texas <- c("This ain't Texas (woo),", 
           "ain't no hold 'em (hey)/",
           "So lay your cards down, down, down, down")
```

a.  Use `str_length` to determine how many characters are in each string
...
d.  Use `str_subset` to select the string that have words in parentheses. Use the following regex as the pattern to match: `\\((.*?)\\)`
...
#b
str_count(texas, "down")

#c
str_detect(texas, "Texas")
...
Dugouts are human-made water resevoirs on the landscape, often used for cattle or other ranching ventures.
...
a.  Using `filter` and `str_detect`, return rows that have "slight" in the `SoilSalinity` column
b.  Using `filter` and `str_detect`, return rows that have a letter in the `Site_ID` column. The pattern to match is `"[A-Z]+"`.

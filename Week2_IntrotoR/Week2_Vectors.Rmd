...
This happens with most calculations return `NA` when `NA` is in the data. Thankfully, we can tell many functions to remove the `NA` before calculating.
...
Let's build on example where we have information on states and population counts by adding areas, all stored in vectors.
...
When do this multiplication, R multiplies the first value in the vector by 2, then multiplies the second values in the vector by 2, and so on. This is called _element-wise_, operating on one element of the vector at a time.

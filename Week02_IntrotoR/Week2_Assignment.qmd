...
author: ""    # <- Your name here, in quotation marks
date: today   # <- No need to edit; this will produce the correct date
format: pdf
editor: visual
---
...
In the code chunk at the end of the question, type out the following calculations.
...
In your code chunk, be sure to comment the code. Before each expression, add a comment that indicates which exercise it is (e.g., `#1a`, `#1b`). Even better, you can write a short comment that indicates what the code is doing (e.g., `# division`).
...
In the code chunk at the end of the question, create similar code to convert a mass in pounds to a mass in kilograms for a desert cottontail (*Sylvilagus audubonii)*. Remember to comment your code!
...
a.  Create a variable `grams` and assign it the mass of a single *Neotoma albigula* (250g).
...
e.  Print the final answer to the screen. Do this by typing the name of the object created in (d) on the final line of the code.
...
*Optional Challenge:* Do the same thing as task (f) (immediately above), but instead of creating the intermediate variable, perform both the square root and the round on a single line by putting the `sqrt()` call inside the `round()` call. Putting functions inside of other functions is called *nesting*.
...
At the end of this question, add a new code chunk (`Ctrl + Atl + I` or `Cmd + Opt + I`).

In the new code chunk, add additional lines of code come accomplish the following:
...
c.  The total NPP over a year for the two sites combined (the sum of the total daily NPP values from the first code chunk multiplied by 365).
...
Now, add a new code chunk and write code that accomplishes the following:
...
Add a new code chunk and determine the following:

a.  The volume of each shrub (length × width × height). *Storing this output in a variable will make some of the next problems easier.*
...
In Question 3, we used the variable names `grams`, `number`, and `biomass` to describe the individual mass, number of individuals, and total biomass of some white-throated woodrats. But are these variable names the best choice?
...
Follow these steps to successfully turn in your assignment on Brightspace/D2L.
...
2.  Click the empty box to the left of the PDF file in the `Files` panel.
...
5.  Head over to Brightspace/D2L and navigate to the correct Assignment dropbox. Submit the PDF file that you just downloaded.

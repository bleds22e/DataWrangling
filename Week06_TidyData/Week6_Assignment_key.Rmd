...
```{r}
forest <- read_csv("data/forest_per_country.csv", skip = 4, col_names = TRUE)
```
...
```{r}
oecd <- read_csv("data/oecd_annual_data.csv", skip = 4, col_names = TRUE)
head(oecd)
```
...
```{r}
rodents <- read_csv("data/capture_data.csv")
```
...
```{r}
veg <- read_csv("data/microsite_grouped_veg.csv")

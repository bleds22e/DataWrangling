...
In today's lesson, we will be talking about how to bring multiple data frames together into one data frame.
...
To start, let's read in the `surveys` data frame from the Portal dataset.
...
![](images/binds.png){width="50%" fig-align="center"}
...
The `surveys` data only goes through 2002, but we have a lot more data from the site! Let's pull down all of the rodent data since 2002 from the [`portalr` package](https://weecology.github.io/portalr/), a package the Weecology lab made to make working with the (actual) Portal data a bit easier. Run the following code chunk.

```{r}
# load our package
# we can ignore the message produced
library(portalr)
...
The magic comes because they merge datasets by *matching up rows of data based on unique identifiers ("keys") in the specified columns*.
...
![](images/joins.png){width="50%" fig-align="center"}

You can find another way to visualize what happens during different type of joins through GIFs from [tidyexplain](https://www.garrickadenbuie.com/project/tidyexplain/).

To enable us to make these connections, the tables need one (or more) columns that link them together; these are called *primary keys*.
...
Looking at the combined table, we can see that on every row in `rodent_detail` with a particular value for `RodentSpeciesID` column, the join has added the matching values from `rodent_species`, including the species code, scientific name, and family.
...
Note: for our assignment, we’ll focus on using `inner_join`.
...
If it helps you to keep track of which data tables are being joined when, you can also use the placeholder for your chosen pipe (`.` for `%>%`; `_` for `|>`).

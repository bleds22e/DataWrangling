...
We want to bind these two data frames together so that we have *all* of the rodent data in one data frame. To do so, we would use `bind_rows()`. Even though columns are getting matched up, the things we are binding together are multiple rows, hence `bind_rows()`.
...
Let’s join the surveys and the plots tables together using an “inner join.”
...
Scroll to Line 324 in the `surveys` table; these have missing species IDs. If you look in the `combined` table, you'll notice that `record_id`, 324-326 are missing from the table.

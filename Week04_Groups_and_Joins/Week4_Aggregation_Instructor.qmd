...
![](images/split-apply-combine.png){width="75%" fig-align="center"}
...
The `group_by()` function doesn't seem to change the data frame visually in any way. 

However, you will notice that next to the information about the tibble (number of rows and columns), there is now an additional bit of information that tells us that this is now a grouped data frame: grouped by the year column, and there are 26 groups.
...
Fortunately, the `dplyr` has a handy-dandy function to make this easy to do with data frames. The `summarize()` function creates a new data frame with columns and values we give it.
...
```{r}
surveys %>%
  # create groups based on year and species
